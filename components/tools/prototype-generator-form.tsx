...
import { PrototypeSpec, PrototypePlatform } from '@/lib/tools/prototype-spec';
...
const pageTypes = ['表单页', '列表页', '详情页', 'Dashboard', '设置页', '其他'];
...
export function PrototypeGeneratorForm() {
  ...
  const [pageType, setPageType] = useState('详情页');
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setStatusText(data.data.usedAI ? `已生成，模型：${data.data.model}` : '已生成基础版原型');
    } catch (error) {
  ...
  const handleRevise = async () => {
    ...
    try {
      ...
      setStatusText(data.data.usedAI ? `已生成 V${data.data.version}，模型：${data.data.model}` : `已生成 V${data.data.version} 基础版`);
    } catch (error) {
      ...
      <section className="xl:col-span-2 rounded-lg border bg-card/70 p-5">
        <div className="flex items-center gap-2 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">填写页面需求，生成可导入 Figma 的中保真原型</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              ...
              <select
                value={platform}
                onChange={(event) => setPlatform(event.target.value as PrototypePlatform)}
          ...
          <label className="block">
            ...
            <textarea
              ...
              placeholder="描述产品、业务场景和当前问题"
              className="mt-2 w-full min-h-24 rounded-lg border border-input bg-background px-3 py-2 text-sm leading-relaxed focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
            />
          </label>
          ...
          <label className="block">
            ...
            <textarea
              ...
              placeholder="例如：会员状态、权益列表、三档续费方案、提醒设置、底部按钮"
              className="mt-2 w-full min-h-24 rounded-lg border border-input bg-background px-3 py-2 text-sm leading-relaxed"
            />
          </label>
          ...
          <label className="block">
            ...
            <textarea
              ...
              placeholder="例如：中保真移动端页面，突出主按钮，整体简洁清晰"
              className="mt-2 w-full min-h-24 rounded-lg border border-input bg-background px-3 py-2 text-sm leading-relaxed"
            />
          </label>
          ...
          <div>
            ...
            {previewUrl ? (
              ...
              <button
                ...
                className={cn(
                  'w-full rounded-lg border border-dashed bg-background/60 py-8',
                  'flex flex-col items-center justify-center gap-2 text-muted-foreground hover:text-primary hover:border-primary/40 transition-colors'
                )}
              >
      ...
      <section className="xl:col-span-3 rounded-lg border bg-card/70 p-5 min-h-[720px] flex flex-col">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">网页内只读预览；可复制导入码到 Figma 插件生成可编辑图层。</p>
          </div>
          {versions.length > 0 ? (
            <div className="flex flex-wrap justify-end gap-2">
              {versions.map((item) => (
                <button
                  key={item.specId}
                  type="button"
                  onClick={() => setSelectedSpecId(item.specId)}
                  className={cn(
                    'rounded-full border px-3 py-1 text-xs transition-colors',
                    selectedVersion?.specId === item.specId
            ...
            <div className="rounded-lg border bg-background p-4">
              <div className="mb-3 flex items-start justify-between gap-4">
                ...
                <span className="rounded-full bg-muted px-3 py-1 text-xs text-muted-foreground">
                  {formatExpiresAt(selectedVersion.expiresAt)} 过期
                </span>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <code className="flex-1 rounded-lg bg-muted px-3 py-2 text-sm tracking-widest">
                  {selectedVersion.importCode}
                </code>
            ...
            <div className="rounded-lg border bg-background p-4">
              <label className="block">
                ...
                <textarea
                  ...
                  placeholder="例如：把底部按钮改成固定悬浮；新增会员权益对比卡片；整体更像 B 端后台页面"
                  className="mt-2 w-full min-h-24 rounded-lg border border-input bg-background px-3 py-2 text-sm leading-relaxed"
                />
              </label>
