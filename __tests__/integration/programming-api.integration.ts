/**
 * 编程知识训练 API 集成测试。
 * 需要先启动 http://localhost:3000，因此不纳入默认 `npm test`。
 */
...
type ProgrammingQuestionListItem = {
  domain: string;
  difficulty: string;
...
describe('编程知识训练 API 测试', () => {
  ...
  describe('题目查询 API', () => {
    it('应该能按领域筛选题目', async () => {
      ...
      expect(data.data.questions.every((q: ProgrammingQuestionListItem) => q.domain === 'frontend')).toBe(true);
    });
    ...
    it('应该能按难度筛选题目', async () => {
      ...
      expect(response.ok).toBe(true);
      expect(data.data.questions.every((q: ProgrammingQuestionListItem) => q.difficulty === 'beginner')).toBe(true);
    });
