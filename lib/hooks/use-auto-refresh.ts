...
const STORAGE_KEY = 'pm-hub-article-version';
const LAST_REFRESH_KEY = 'pm-hub-last-refresh';
...
export function useAutoRefresh(options: UseAutoRefreshOptions = {}): UseAutoRefreshResult {
  ...
  // 获取最新文章状态
  const fetchStatus = useCallback(async (): Promise<ArticleStatus | null> => {
    try {
  ...
  const checkForUpdates = useCallback(async () => {
    ...
    try {
      ...
      console.error('Auto-refresh check failed:', message);

      // 重试逻辑
      retryCountRef.current++;
      if (retryCountRef.current < maxRetries) {
        setError(`检查失败，${retryDelay / 1000}秒后重试...`);
        checkTimeoutRef.current = setTimeout(checkForUpdates, retryDelay);
      } else {
        setError(message);
